...
A from-the-ground-up **hybrid foundational model** architecture (**NUERONCE**) plus a
companion **research MCP server**.

NUERONCE reorganizes the standard `next_token = MODEL(previous_tokens)` recipe into a
...
all of it via token continuation. See **[`docs/NUERONCE_design.md`](docs/NUERONCE_design.md)**
for the full design and **[`docs/architecture.md`](docs/architecture.md)** for the
module map.

Every operator is **built from scratch** (`nueronce/nn.py`): PyTorch is used only as
...
nueronce/
  ...
  blocks.py               NUERONCE operators: byte CNN perception, typed recurrent
                          memory, hybrid SSM/attention block, byte decoder
  segment.py              Dynamic patching → segment ids, pooling, causal masks
  model.py                NUERONCEModel — the trainable two-level byte LM
  ...
  engine/               From-scratch (NumPy-only) tensor + autograd engine;
                            nueronce_model.py/nueronce_blocks.py/segment.py port the
                            *full* NUERONCE architecture onto it (see below) —
...
python scripts/compare_baselines.py      # NUERONCE vs Transformer/SSM on held-out bytes
```
...
python scripts/train_checkpoint.py --minutes 20   # corpus -> NUERONCE weights (+ held-out bpb)
...
dominates, and whole books are held out for validation. See `nueronce/corpus/`.
...
set in `nueronce/training/dialogue_data.py`, masking the loss to response bytes
only. It works with two interchangeable backends — `nueronce.training.sft.
TorchSFTBackend` (PyTorch, fine-tunes `NUERONCEModel`) and `nueronce.engine.
models.MicroSFTBackend` (the from-scratch, NumPy-only engine described under
`nueronce/engine/`, so this stage runs with no PyTorch dependency at all).
...
`nueronce/engine/` is a hand-built (NumPy-only) tensor + reverse-mode autograd
...
PyTorch parity (`tests/test_nueronce_engine.py`).

`nueronce/engine/nueronce_model.py` (+ `nueronce_blocks.py`, `segment.py`) is a
module-for-module port of the *real* `NUERONCEModel` onto that engine — the same
...
stand-in. `NueronceModel` matches `NUERONCEModel`'s interface (`forward`, `loss`,
...
`nueronce.engine.models.MicroSFTBackend`, so the full production architecture
— not just the smaller `MicroByteLM` demo — can be dialogue-SFT-tuned with
zero PyTorch dependency. See `tests/test_nueronce_engine_nueronce_model.py`.

```python
import torch
from nueronce.model import NUERONCEModel, ModelConfig

model = NUERONCEModel(ModelConfig())                 # ~2M params, all hand-built
ids = torch.randint(0, 256, (2, 96))
loss, parts = model.loss(ids)                    # next-byte CE + boundary loss
loss.backward()
print(parts["bpb"], "bits/byte")
print(model.generate(b"NUERONCE separates ", max_new=40, greedy=True))
```
...
take "from scratch" to mean the whole NUERONCE design is built; it isn't yet.
...
[`docs/NUERONCE_design.md`](docs/NUERONCE_design.md).
