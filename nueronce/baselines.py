"""Matched, from-scratch baselines for honest comparison against NUERONCE.

Built from the same hand-rolled primitives (:mod:`nueronce.nn`) — no stock
...
``NUERONCEModel``: ``forward``, ``lm_loss``, ``num_params``.

The point of these is the reviewer's Priority 1: without matched baselines and a
held-out set you cannot tell whether NUERONCE's machinery helps. These let you
