"""NUERONCE operator modules on the from-scratch Nueronce Engine — a faithful
port of ``nueronce.blocks`` (PyTorch) so the real production architecture (not
just a smaller demo model) can run on nothing but NumPy.

Mirrors ``nueronce.blocks`` module-for-module:
...
class TypedRecurrentMemory(Module):
    """Typed gated recurrence over a unit sequence (see ``nueronce.blocks`` for the
...
class HybridBlock(Module):
    ...
    def __init__(self, d_model: int, n_heads: int = 4, local_window: int = 32,
        ...
        self.ssm = NUERONCESelectiveSSM(d_model, d_state=d_state)
