"""Conversation interface for a trained NueronceModel checkpoint — the
engine port of ``nueronce.chat`` (which needs PyTorch to load a ``NUERONCEModel``
...
signal requires an SFT pass (``nueronce.training.sharded_sft`` /
``nueronce.training.sft``), same as the PyTorch path.

Checkpoints here are the ``nueronce.training.sharded_sft.save_checkpoint`` pickle
format (reused directly, not re-implemented) rather than ``nueronce.chat``'s
...
from .nueronce_model import NueronceModel, NueronceConfig

_STOP = b"\n"


def load_checkpoint(path: str) -> Tuple[NueronceModel, dict]:
...
class MicroConversation:
    ...
    ``"canonical"`` = the ``<|user|>``-marker format from ``nueronce.prompting``;
