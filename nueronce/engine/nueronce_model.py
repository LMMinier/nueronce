...
class NueronceConfig:
    ...
    strict_segmentation_capacity: bool = False
...
def _softmax_np(v: np.ndarray) -> np.ndarray:
    v = np.nan_to_num(np.asarray(v, dtype=np.float64), nan=-1e30, posinf=1e30, neginf=-1e30)
    e = np.exp(v - v.max())
    total = e.sum()
    if not np.isfinite(total) or total <= 0.0:
        out = np.zeros_like(e)
        out[int(v.argmax())] = 1.0
        return out
    return e / total


def _decode_next_byte(
    logits: np.ndarray,
    *,
    ids: List[int],
    generated: List[int],
    ...
    """Apply inference-only decoding controls without altering training loss.

    The repetition penalty matches the primary PyTorch backend: before any byte
    is generated it considers the prompt, then considers generated bytes only.
    A value of 1.0 preserves exact historical behavior.
    """
    if repetition_penalty <= 0.0:
        raise ValueError("repetition_penalty must be positive")
    scores = np.nan_to_num(
        np.asarray(logits, dtype=np.float64).copy(),
        nan=-1e30,
        posinf=1e30,
        neginf=-1e30,
    )
    ...
    if ngram_size and len(generated) >= ngram_size - 1:
        prefix = tuple(generated[-(ngram_size - 1):]) if ngram_size > 1 else ()
        blocked = set()
        for start in range(len(generated) - ngram_size + 1):
            if tuple(generated[start:start + ngram_size - 1]) == prefix:
                blocked.add(generated[start + ngram_size - 1])
        for token in blocked:
            scores[token] = -1e30
    if greedy:
        return int(scores.argmax())

    scaled = scores / max(1e-5, temperature)
    if top_k is not None:
        k = max(1, min(int(top_k), scaled.size))
...
class NueronceModel(Module):
    ...
    def encode_units(self, byte_ids: np.ndarray, ret_ctx: Optional[Tensor] = None,
        ...
        seg_ids, _ = segment.segment_ids_from_boundaries(
            prob_hard, tau=c.tau, min_patch=c.min_patch, max_patch=c.max_patch,
            p_max=c.p_max,
            strict_capacity=getattr(c, "strict_segmentation_capacity", False),
        )
        ...
        with no_grad():
            for _ in range(max_new):
                ...
                idx = _decode_next_byte(
                    nxt, ids=ids, generated=generated, temperature=temperature,
