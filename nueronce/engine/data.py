"""NumPy port of the parts of ``nueronce.data`` the held-out eval harness needs
(``nueronce.data`` imports torch unconditionally at module scope, so it can't be
...
The corpus text is copied verbatim from ``nueronce.data.LARGER_CORPUS`` so a
...
# Verbatim copy of nueronce.data.LARGER_CORPUS — see that module for the source of
# truth; duplicated here only because nueronce.data cannot be imported without torch.
LARGER_CORPUS = (
