"""Neural building blocks on the Nueronce Engine.

Mirrors the NUERONCE primitives (nueronce/nn.py) but on the from-scratch autograd engine,
...
class MultiHeadAttention(Module):
    ...
    the same content-addressed mixing NUERONCE uses, hand-built on the engine."""
...
class SparseGlobalAttention(Module):
    ...
    ``nueronce.nn.SparseGlobalAttention``."""
...
class CrossAttention(Module):
    ...
    stream, matching ``nueronce.nn.CrossAttention`` (byte->unit decoding,
    retrieval injection)."""
...
class NUERONCESelectiveSSM(Module):
    """Faithful port of ``nueronce.nn.SelectiveSSM``: input-dependent (selective)
    ...
    kept for ``MicroByteLM``/``scripts/nueronce_engine_demo.py`` backward compat).
...
__all__ = ["Parameter", "Module", "Linear", "Embedding", "RMSNorm", "MLP",
           ...
           "SelectiveSSM", "NUERONCESelectiveSSM"]
