"""State-cached incremental generation for the PyTorch ``NUERONCEModel`` — the
torch twin of :mod:`nueronce.engine.incremental`, exploiting the same three
...
byte-identical to ``NUERONCEModel.generate`` and per-step logits must match to
...
context should use the dense path (``nueronce.chat`` falls back automatically).
"""
...
class IncrementalGenerator:
    ...
    def generate(self, prompt, *, max_new: int = 256, max_ctx: Optional[int] = None,
        ...
        """Mirrors the dense ``NUERONCEModel.generate`` contract for the supported
