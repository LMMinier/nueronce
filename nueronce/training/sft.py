...
real, PyTorch-backed :class:`~nueronce.model.NUERONCEModel`.

``nueronce.chat`` is honest that an untuned checkpoint gives "English-shaped
...
:mod:`nueronce.training.dialogue_data` (the same ``User: `` / ``Assistant: ``
turn layout ``nueronce.chat.Conversation`` uses at inference) and fine-tunes an
already-pretrained ``NUERONCEModel`` with the loss masked to the response bytes
...
See :mod:`nueronce.engine.models` for the same idea running on the from-scratch
...
def make_sft_batch(examples: Sequence[SFTExample], **kwargs) -> Dict[str, torch.Tensor]:
    """``dialogue_data.make_sft_batch`` as torch tensors, ready for NUERONCEModel."""
...
def sft_step(model: NUERONCEModel, opt: torch.optim.Optimizer, batch: Dict[str, torch.Tensor]) -> float:
    model.train()
...
@torch.no_grad()
def sft_eval(model: NUERONCEModel, batch: Dict[str, torch.Tensor]) -> float:
    model.eval()
    logits, _ = model(batch["byte_ids"])
    return float(model.masked_token_loss(logits, batch["byte_ids"], batch["target_mask"]).item())


def train_sft(model: NUERONCEModel, opt: torch.optim.Optimizer, examples: Sequence[SFTExample],
    ...
    loss to response bytes only. Mirrors ``nueronce.retrieval_train.train_retrieval``:
...
class TorchSFTBackend:
    ...
    a ``NUERONCEModel`` + optimizer pair, trained with PyTorch autograd. Stages 2-4
