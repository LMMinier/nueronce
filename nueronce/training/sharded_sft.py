...
``NueronceModel`` at real scale (10 shards x 10,000 conversations = 100,000),
entirely on the from-scratch Nueronce Engine (no PyTorch anywhere).
...
from ..engine.nueronce_model import NueronceModel, NueronceConfig
from ..engine.optim import AdamW, clip_grad_norm_
from ..engine.tensor import no_grad
...
def evaluate(model: NueronceModel, records: List[dict], *, batch_size: int = 64,
...
def save_checkpoint(path: str, model: NueronceModel, opt: AdamW, meta: dict) -> None:
...
def new_model_and_optimizer(cfg: NueronceConfig, lr: float, seed: int):
    np.random.seed(seed)
...
def apply_checkpoint(payload: dict, model: NueronceModel, opt: AdamW) -> None:
...
def run_sharded_sft(model_cfg: NueronceConfig, cfg: ShardedSFTConfig,
                    log_fn=print) -> dict:
    ...
    best_model = NueronceModel(NueronceConfig(**best_payload["config"]))
