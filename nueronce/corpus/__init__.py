"""Open, license-clean corpus acquisition for NUERONCE's first real checkpoint.
...
def __getattr__(name):
    if name in {"sources", "build", "dataset", "stack", "catalog"}:
        return importlib.import_module(f".{name}", __name__)
    raise AttributeError(f"module 'nueronce.corpus' has no attribute {name!r}")


__all__ = ["sources", "build", "dataset", "stack", "catalog"]
