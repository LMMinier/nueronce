"""Recommended staged corpus stack for NUERONCE training.
...
CORPUS_STACK: List[CorpusStackEntry] = [
    ...
    CorpusStackEntry(
        ...
        target_notes="Step-by-step math answers; convert via nueronce.training.mcq_sft (qa mode).",
    ),
    # ------------------------------------------------------------------ #
    # MCQ instruction tuning (phase 4) — convert via nueronce.training.mcq_sft
    # ------------------------------------------------------------------ #
    CorpusStackEntry(
        source_id="arc_easy",
        name="AI2 ARC (Easy)",
