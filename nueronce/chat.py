"""Conversation interface for a trained NUERONCE byte checkpoint.
...
``scripts/train_sft.py`` (backed by ``nueronce.training.sft`` /
``nueronce.training.vgrft.VGRFTTrainer``) adds an actual supervised fine-tuning
...
from .model import NUERONCEModel, ModelConfig
...
@torch.no_grad()
def _continue(model: NUERONCEModel, context: bytes, max_new: int, temperature: float,
              stop_bytes: bytes, min_new: int, max_ctx: int,
              use_incremental: bool = True) -> bytes:
    del stop_bytes, min_new
    if use_incremental:
        # State-cached generation (nueronce.incremental): the engine must prove
...
class Conversation:
    ...
    def _context(self, user_msg: str) -> bytes:
        # Match nueronce.training.dialogue_data.encode_messages exactly for
