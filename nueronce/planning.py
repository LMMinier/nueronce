...
NUERONCE separates *what to say* (a structured plan with evidence map, uncertainty
...
class CausalLanguageRenderer:
    ...
    Backed by a real byte model (``nueronce.model.NUERONCEModel`` or anything exposing
    ``generate(prompt: bytes, max_new, greedy) -> bytes``). The draft's
    ``"prompt"`` seeds generation; the full pipeline uses
    ``nueronce.pipeline.ModelRenderer`` which is the same contract.
    """
    ...
    def render(self, semantic_draft: dict, style: dict) -> str:
        if self.model is None:
            raise needs_backend(
                ...
                "Pass a trained model (NUERONCEModel) to render; see nueronce.pipeline.",
            )
