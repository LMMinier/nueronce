# CFNA Hybrid Foundational Model — Implementation Map and Pseudocode

> This document is the design source of truth for the `cfna` package. It is the
> implementation map supplied for the project. The code under `cfna/` implements
...
The CFNA proposal is not a request for "a better Transformer." It is a request
...
| Best CFNA role | Perception / boundary finding | Relation routing / sparse comparison | Typed persistent state |

### Retrieval representation split

| Representation | Strength | Weakness | CFNA use |
|---|---|---|---|
...
## Assumptions and CFNA-350M defaults

These are *implementation defaults for a minimal reproducible prototype*, not
values fixed by the source notes. They live in `cfna/config.py`.
...
Implemented in `cfna/types.py`: `SourceRecord`, `KnowledgeUnit`,
...
JSON schemas and worked examples for the wire records live in `cfna/schemas/`.

## Subsystem notes

### Ingestion + provenance gate (`cfna/ingestion.py`)
...
### Parser + knowledge-unit compiler (`cfna/parsing.py`)
...
### Multiscale perception + dynamic patching (`cfna/perception.py`)
...
aggregation/registry (`cfna/training/curriculum.py`, `episodes.py`, `losses.py`).
