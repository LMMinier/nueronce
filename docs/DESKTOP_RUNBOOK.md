...
`notebooks/nueronce_colab_training.ipynb`, which is the same pipeline cell by
...
python scripts/train_checkpoint.py --preset base_35m --corpus corpus_stack \
    ...
    --out checkpoints/nueronce_base_35m.pt
```
...
python scripts/train_conversation.py --data data/conversation_sft \
    --preset base_35m --init-from checkpoints/nueronce_base_35m.pt \
