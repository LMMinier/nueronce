# MicroCFNA 100K-Conversation SFT Run — Report
...
run on the **same, unenlarged** `MicroCFNAModel` architecture (112,709 params —
identical config to the 61-example run), entirely on the from-scratch
`cfna/microtorch` engine (no PyTorch anywhere in the training path). The
...
parameters (`cfna/training/synthetic_dialogue.py`), not a scraped or third-party
...
the full manifest and `cfna/training/dataset_prep.py` for the pipeline.
...
```
MicroModelConfig(byte_embed_dim=16, d_local=24, d_model=32, p_max=16,
...
- Full architecture used: `BytePerceptionEncoder` (causal byte CNN + boundary
  head) → dynamic patching (`cfna/microtorch/segment.py`) → `UnitEmbedder` →
  `TypedRecurrentMemory` → `HybridCoreStack` (`CFNASelectiveSSM` + local
  ...
  `tests/test_microtorch_cfna_model.py::test_retrieval_path_only_gets_gradient_when_used`)
...
- Loss: `masked_token_loss`, response-bytes-only (including the terminal stop
  byte), computed via `cfna.training.dialogue_data.encode_messages`. Verified
...
```bash
python scripts/train_sft.py --backend microtorch --model full-cfna \
  ...
  --save-dir checkpoints/micro_cfna_sft_100k --metrics-dir metrics \
...
PyTorch installed) — this run is a genuine test of the microtorch engine's
...
**Best checkpoint: shard 10** (`checkpoints/micro_cfna_sft_100k/best.pt`),
...
tests). Combined with the pre-existing `tests/test_microtorch_cfna_model.py`
and `tests/test_sft_microtorch.py`, all 16 required categories are covered:
...
| 13 | Small end-to-end microtorch SFT learning | `test_sft_microtorch.py::test_micro_dialogue_sft_loss_decreases_on_a_toy_set` |
| 14 | Full MicroCFNAModel forward/backward | `test_microtorch_cfna_model.py::test_forward_and_loss_shapes`, `test_model_learns_on_toy_corpus` |
...
| 16 | Retrieval behavior | `test_microtorch_cfna_model.py::test_retrieval_path_only_gets_gradient_when_used` |
...
`scripts/eval_generalization.py` against `checkpoints/micro_cfna_sft_100k/best.pt`:
100 conversations sampled verbatim from training (memorization probes, gold
answers known) + 311 hand-authored novel prompts
(`cfna/training/generalization_eval.py::build_novel_prompts`) — new operand
