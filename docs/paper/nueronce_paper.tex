...
% Figure placeholders compile standalone; replace each \figbox with an
% \includegraphics of the rendered diagram before submission.
\newcommand{\figbox}[1]{\fbox{\parbox[c][1.6in][c]{0.94\linewidth}{\centering\footnotesize #1}}}
...
\begin{figure}[!t]
  \centering
  \figbox{FIGURE 1 (to render): End-to-end CFNA forward pass. Byte stream $\to$ causal perception CNN (embed, conv-3, residual conv-7, residual dilated conv-3, RMSNorm) with boundary head $\to$ dynamic patcher (learned boundary probability thresholded, min/max patch lengths) $\to$ unit embedder $\to$ typed recurrent memory $\to$ hybrid routed core (reused over logical depth) $\to$ byte decoder cross-attending to completed units $\to$ 256-way next-byte logits. Annotate the two gradient routes into the boundary head.}
...
\begin{figure}[!t]
  \centering
  \figbox{FIGURE 2 (to render): A single hybrid core block. One normalized input fans out to four parallel paths; a router MLP consumes the concatenated outputs and emits a per-position four-way softmax blend; gated feed-forward with residual follows. Lock icon: the router sees only causal path outputs.}
...
\begin{figure}[!t]
  \centering
  \figbox{FIGURE 3 (to render): Dual-backend equivalence harness. Two columns (framework backend; NumPy float64 autograd engine) realize identical architecture, optimizer, eval harness, authority classifier; a comparator checks serialized benchmark output for bit-identical equality across OS, Python version, and backend chips.}
...
\begin{figure}[!t]
  \centering
  \figbox{FIGURE 4 (to render): Base pretraining learning curve. Held-out bits/byte vs.\ minutes, 4.515 down through annotated milestones (2.67, 2.11, 2.03), crossing the dashed pre-registered 1.8 gate at minute 51, ending 1.785 at cutoff; a later-session extension to 1.313 marked with a broken-link icon (checkpoint lost). Train-bpb overlay near 1.2 at cutoff.}
