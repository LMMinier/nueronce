...
\hypersetup{
  pdftitle={NUERONCE: A From-Scratch, Byte-Level, Provenance-Gated Language Model},
...
\title{NUERONCE: A From-Scratch, Byte-Level, Provenance-Gated\\
...
%%% ------------------------------------------------------------------
\IEEEPARstart{T}{he} dominant recipe for large language models fixes a subword vocabulary in advance, embeds tokens into a single dense stream, and processes that stream with stacked dense-attention transformer blocks; the scaling report of Brown \emph{et al.}~\cite{Brown2020} is the canonical demonstration that capability emerges predictably with parameters and data. NUERONCE starts from a different set of design commitments and asks how far a very small model built on them can be pushed with a modest, license-clean corpus and honest bookkeeping.
...
\begin{figure}[!t]
  ...
  \caption{The end-to-end NUERONCE forward pass. The discrete segmentation is detached from the gradient; the boundary head is trained by the auxiliary word-onset loss and by the language-model loss through a per-unit mean-boundary feature.}
  \label{fig:pipeline}
\end{figure}
...
ByT5~\cite{Xue2022} established that token-free byte models are viable and noise-robust at the cost of longer sequences. MegaByte~\cite{Yu2023} introduced the two-level patch/byte decomposition that NUERONCE's unit-core/byte-decoder split belongs to. The Byte Latent Transformer~\cite{Pagnoni2024} is closest in spirit with dynamic, entropy-driven patching; NUERONCE's patcher instead thresholds a single learned boundary probability trained jointly by word-onset self-supervision and the language-model gradient, with the discrete cuts detached. MambaByte~\cite{Wang2024} demonstrated byte modeling on a state-space backbone; NUERONCE uses a selective scan as one of four routed paths rather than the sole mixer. Unlike all of these, NUERONCE has not been evaluated on a standardized byte benchmark; its placement here is architectural, not results-based.

\subsection{State-Space and Hybrid Sequence Models}
Mamba~\cite{Gu2023} motivates the recurrent path, implemented by hand as an input-dependent selective scan. Jamba~\cite{Lieber2024} and Griffin~\cite{De2024} validate mixing recurrence with attention, but interleave block types by depth; NUERONCE computes all four mixers in every block and blends them per position with a learned router. Whether per-position fusion beats a plain interleave at equal compute is untested.

\subsection{Tokenizer-Based Frontier Designs}
The GPT-3~\cite{Brown2020}, PaLM~\cite{Chowdhery2022}, and LLaMA~\cite{Touvron2023} line is the backdrop for NUERONCE's departures; Chinchilla's compute-optimality lesson~\cite{Hoffmann2022} is encoded directly in the parameter-ladder presets (scale data with parameters; drop learning rate roughly with inverse width). The retrieval path draws on the RETRO lineage~\cite{Borgeaud2022}, kept at byte level so exact identifiers stay copyable. The typed memory descends conceptually from the protected cell state of long short-term memory~\cite{Hochreiter1997}. The calibration methodology for the authority classifier follows~\cite{Guo2017}.
...
Table~\ref{tab:process} states, stage by stage, where NUERONCE's process departs from the established LLM production recipe. Every entry in the NUERONCE column is implemented and testable in the repository; none is aspirational. The table lists verified process differences, not superiority claims---the equal-compute comparison that could support a superiority claim has not been run.

\begin{table*}[!t]
\caption{The Established LLM Process vs.\ What NUERONCE Verifiably Does}
\label{tab:process}
\centering
\small
\begin{tabularx}{\textwidth}{lYY}
\toprule
\textbf{Stage} & \textbf{Established process} & \textbf{NUERONCE (verified in code)} \\
\midrule
