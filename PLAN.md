# PLAN: NUERONCE — from "English-shaped continuations" to a conversational model, then scale

**Mission.** Take NUERONCE from a model that produces English-shaped continuations
...
112,709-param `micro_nueronce_sft_100k` proof-of-loop at 0.906 teacher-forced byte
...
|---|---|---|
| Canonical markers `<\|system\|>` `<\|user\|>` `<\|evidence\|>` `<\|plan\|>` `<\|assistant\|>` `<\|end\|>` | DONE | `nueronce/prompting.py` |
...
| Synthetic dialogue generator (~127K unique) | DONE | `nueronce/training/synthetic_dialogue.py` |
| MCQ/QA conversion (ARC, OpenBookQA, CommonsenseQA, MathQA, GSM8K) | DONE | `nueronce/training/mcq_sft.py` (`load_and_convert`, `convert_records`) |
...
| Item | Status | Evidence |
|---|---|---|
| Stage-1 SFT (response-only masked CE), torch backend | DONE | `nueronce/training/sft.py` |
| Stage-1 SFT, sharded/resumable + engine backend | DONE | `nueronce/training/sharded_sft.py`; `tests/test_sft_engine.py` |
| Proof the loop works end-to-end | DONE | `checkpoints/micro_nueronce_sft_100k` → 0.906 byte-acc; `docs/reports/MICRO_NUERONCE_SFT_100K_REPORT.md` |
| VGRFT stages 2–4 (verifier-guided revision, residual, continual) | TODO (by design) | `nueronce/training/vgrft.py` raises `NotImplementedError` for un-injected backends; not needed until a base model clears the bpb gate |
...
| Item | Status | Evidence |
|---|---|---|
| Parameter presets: chat_11m / base_35m (34.4M) / base_90m (92.1M) / large_337m | DONE | `nueronce.model.CONFIG_PRESETS`; construct+forward verified |
| fp16/AMP-safe numerics (masked softmax, fp32 RMSNorm stats) | DONE | `nueronce/nn.py`; `tests/test_gpu_amp.py` |
| Multi-subject corpus recipe (~400 MB, 22 sources) | DONE | `nueronce/corpus/stack.py`; `docs/LOCAL_TRAINING_PLAYBOOK.md` §1 |
| State-cached incremental generation (edge-inference path) | DONE | `nueronce/incremental.py` + engine twin; byte-exact vs dense (8 tests), **17.0x** measured at 256-byte prompt (`scripts/bench_incremental.py`); wired into both chat paths with self-verifying fallback |
...
| Colab training journal (corpus → parameters → chat) | DONE | `notebooks/nueronce_colab_training.ipynb` (13 cells, resumable, Drive checkpoints, corpus→GitHub cell) |
