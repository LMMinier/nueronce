"""Held-out evaluation harness + matched baselines on microtorch (reviewer
Priority 1), mirroring tests/test_eval_harness.py. No PyTorch needed."""

import numpy as np

from cfna.microtorch.baselines import BaselineConfig, ByteSSMLM, ByteTransformerLM
from cfna.microtorch.cfna_model import MicroCFNAModel, MicroModelConfig
from cfna.microtorch.data import larger_corpus_bytes, make_batches, train_val_split
from cfna.microtorch.eval_harness import compare
...
def test_compare_reports_heldout_for_all():
    ...
    factories = {
        "CFNA": lambda: MicroCFNAModel(MicroModelConfig(
