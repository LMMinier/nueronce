"""VGRFT stage 1 (SFT) on the from-scratch microtorch engine.

No PyTorch import anywhere in this file (or in what it imports) — it proves
the SFT training pass is real using nothing but the hand-built microtorch
autograd engine (see cfna/microtorch/), so it also runs in environments where
...
from cfna.microtorch.models import MicroByteLM, MicroSFTBackend, train_dialogue_sft
from cfna.training.dialogue_data import (
...
def test_masked_cross_entropy_matches_manual_row_filter():
    from cfna.microtorch.tensor import Tensor
...
def test_masked_loss_matches_manual_single_position_cross_entropy():
    from cfna.microtorch.tensor import Tensor, no_grad
...
def test_vgrft_supervised_instruction_tune_runs_with_microtorch_backend():
    np.random.seed(0)
    model = MicroByteLM(d_model=16, n_heads=2, window=8, d_state=6)
