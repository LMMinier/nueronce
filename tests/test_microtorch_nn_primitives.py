"""The hand-built microtorch primitives: shapes, backward, causality,
...
from cfna.microtorch import nn as mnn
...
def test_shapes_and_backward():
    ...
    for mod in [mnn.MultiHeadAttention(D, 4, window=4),
                ...
                mnn.CFNASelectiveSSM(D, d_state=8),
                mnn.GatedMLP(D, 32),
                mnn.MLP(D, 32, D),
                mnn.RMSNorm(D)]:
        y = mod(x)
...
def test_selective_ssm_is_causal():
    np.random.seed(0)
    D = 16
    ssm = mnn.CFNASelectiveSSM(D, d_state=8)
