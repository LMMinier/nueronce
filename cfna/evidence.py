...
Torch-free on purpose: this is the exact code ``cfna.pipeline.respond`` uses
to gate what the neural model may see (extracted verbatim from
``pipeline._candidate_to_memory``), factored out so the microtorch pipeline
(:mod:`cfna.pipeline_micro`), the integrated Wave-2 evaluation, and the
...
def source_to_memory(
    ...
    Verbatim port of ``cfna.pipeline._candidate_to_memory`` semantics:
