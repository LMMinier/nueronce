"""Matched, from-scratch baselines for honest comparison against MicroCFNAModel
— the microtorch port of ``cfna.baselines`` (which needs PyTorch).

Built from the same hand-rolled microtorch primitives (:mod:`cfna.microtorch.nn`)
...
eval interface MicroCFNAModel does: ``forward``, ``lm_loss``, ``num_params``.
"""
...
from .nn import Embedding, GatedMLP, Linear, Module, MultiHeadAttention, Parameter, RMSNorm, CFNASelectiveSSM
...
class _Block(Module):
    def __init__(self, cfg: BaselineConfig, kind: str):
        ...
        if kind == "transformer":
            ...
            self.mix = CFNASelectiveSSM(cfg.d_model, d_state=cfg.d_state)
