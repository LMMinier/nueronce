"""State-cached (KV/state-style) incremental generation for MicroCFNAModel.

``MicroCFNAModel.generate`` recomputes the full forward pass — perception,
...
from .cfna_model import MicroCFNAModel
...
class IncrementalGenerator:
    """Incremental drop-in for ``MicroCFNAModel.generate`` (no retrieval)."""

    def __init__(self, model: MicroCFNAModel, margin: int = 8):
        self.model = model
        c = model.cfg
    ...
    def generate(self, prompt: bytes, max_new: int = 64, temperature: float = 0.8,
        ...
        """API- and output-compatible with ``MicroCFNAModel.generate``.
