"""Source/authority classifier on the from-scratch microtorch engine.

API-identical mirror of the canonical torch implementation in
:mod:`cfna.authority_clf` (same features via :mod:`cfna.authority_features`,
...
run on machines without PyTorch. ``cfna.authority_clf`` aliases to this class
automatically when torch is missing.

Differences from the torch backend, stated honestly:
- Weight init follows microtorch's Linear (normal / sqrt(fan_in)) rather than
