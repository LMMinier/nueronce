...
- ``cfna.training.sft`` — the production path, fine-tunes the real
  ``CFNAModel`` (PyTorch-backed).
- ``cfna.microtorch`` — a from-scratch (NumPy-only) autograd engine; its
  ``MicroByteLM`` can run the same masked-SFT loss with zero external
  dependencies beyond NumPy, which is what ``tests/test_sft_microtorch.py``
  exercises.
"""
...
def held_out_split(examples: Sequence[SFTExample], val_frac: float = 0.2,
    ...
    document-level-holdout spirit as ``cfna.corpus.dataset.ByteCorpus``."""
