"""Convert open MCQ / QA datasets into CFNA SFT conversations, plus a
...
1. **Generative SFT** — each MCQ becomes a ``User:``/``Assistant:`` turn in
   exactly the schema `cfna.training.dataset_prep` validates and
   `cfna.training.sharded_sft` trains on, so the whole existing
...
2. **Choice ranking** — score each candidate answer by the model's masked
   ...
   content — see MICRO_CFNA_SFT_100K_REPORT).
...
def load_and_convert(entry, split: str = "train", limit: Optional[int] = None) -> Iterator[Dict]:
    """Pull a `cfna.corpus.stack.CorpusStackEntry` from Hugging Face and
...
def rank_choices(model, question_prompt: str, choices: Sequence[str],
    ...
    interface (MicroCFNAModel here, CFNAModel on the GPU machine)."""
    losses = []
