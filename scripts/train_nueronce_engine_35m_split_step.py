#!/usr/bin/env python3
"""Two-process, crash-safe response-only SFT for Nueronce Engine NUERONCE base-35M.

Nueronce Engine graphs contain Python backward closures and cannot be safely
...
from nueronce.engine.optim import clip_grad_norm_, validate_finite_gradients
from nueronce.engine.tensor import no_grad
from scripts.train_engine_35m_phased_sft import (
    PHASES,
    PagedFactorState,
    choose_window,
    load_records,
...
PLAN_VERSION = "engine-base35m-split-step-v1"
...
def backward(args) -> None:
    ...
    payload = {
        "format": "engine-base35m-phased-sft-v3-split",
        "config": cfg.__dict__,
