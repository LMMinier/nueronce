#!/usr/bin/env python3
"""Train briefly, then run the full NUERONCE inference pipeline on a query.
...
def train(model: NUERONCEModel, steps: int, seq: int = 96, batch: int = 16, lr: float = 3e-3):
...
def main():
    ...
    ap.add_argument("--query", type=str, default="NUERONCE separates understanding,")
    args = ap.parse_args()
